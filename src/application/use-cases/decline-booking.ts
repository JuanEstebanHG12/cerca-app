import { ManageBookingError, ManageBookingFailureReason } from '../../domain/errors/booking-errors';
import { Booking } from '../../domain/models/booking';
import { DeclineBookingInput } from '../../domain/models/manage-booking';
import { BookingGateway } from '../ports/booking-gateway';

export type DeclineBookingResult = { ok: true; booking: Booking } | { ok: false; reason: ManageBookingFailureReason };

export class DeclineBookingUseCase {
  constructor(private readonly bookingGateway: BookingGateway) {}

  async execute(id: string, input: DeclineBookingInput, accessToken: string): Promise<DeclineBookingResult> {
    try {
      const booking = await this.bookingGateway.decline(id, input, accessToken);
      return { ok: true, booking };
    } catch (error) {
      if (error instanceof ManageBookingError) {
        return { ok: false, reason: error.reason };
      }
      return { ok: false, reason: 'unexpected_error' };
    }
  }
}
